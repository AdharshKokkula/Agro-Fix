  ...
  private async createSampleOrder() {
    try {
      ...
      const sampleOrder: InsertOrder = {
        ...
        totalAmount:
          products[0].price * products[0].minOrderQuantity +
  ...
  private initializeProducts() {
    const sampleProducts: InsertProduct[] = [
      ...
      },
      {
        ...
        minOrderQuantity: 20,
        imageUrl:
          "https://images.unsplash.com/photo-1587049716454-0136927d45f3",
...
export class DatabaseStorage implements IStorage {
  ...
  private async initializeAdminUser() {
    try {
      ...
      // Create admin user with fixed credentials
      const password = "admin123"; // Store plain text password
      const now = new Date().toISOString();
  ...
  private async createSampleOrder() {
    try {
      ...
      const totalAmount =
        p1.price * p1.minOrderQuantity + p2.price * p2.minOrderQuantity;
      ...
      const order = await this.createOrder(sampleOrder);
      console.log(
        `Sample order created with ID: ${order.id} and number: ${order.orderNumber}`
      );
    } catch (error) {
      console.error("Error creating sample order:", error);
    }
